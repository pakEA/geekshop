...
class OrderListView(BaseClassContextMixin, ListView):
    model = Order
...
class OrderCreateView(BaseClassContextMixin, CreateView):
    model = Order
    fields = []
    ...
    def get_context_data(self, **kwargs):
        ...
        OrderFormSet = inlineformset_factory(Order, OrderItem, OrderItemsForm,
                                             extra=1)
...
class OrderUpdateView(BaseClassContextMixin, UpdateView):
    model = Order
    fields = []
    success_url = reverse_lazy('orders:list')
...
class OrderDeleteView(BaseClassContextMixin, DeleteView):
    model = Order
    success_url = reverse_lazy('orders:list')
    title = 'Geekshop | Удаление заказа'


class OrderDetailView(BaseClassContextMixin, DetailView):
    model = Order
